...
def print_lane():
    ...
    img_dir = "Txt file containing test set pictures"
    save_dir = " your Storage location"
    # Loading model
    ...
    for i in range(len(demo)):
        demo[i]=demo[i][:-1]
    for count in range(0,len(demo),4):
        n_clusters = torch.tensor([2,2,2,2])
        size = (512,256)
        # Load data
        image_data,image_cv_list = read_multi_image(demo[count:count+4],size)
        # Output results
        with torch.no_grad():
